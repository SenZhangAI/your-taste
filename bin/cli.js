...
if (command === 'insights') {
  ...
  console.log('  export [path]     Export standalone CLAUDE.md (no plugin needed)');
...
async function runExport() {
  const tasteDir = process.env.YOUR_TASTE_DIR || `${process.env.HOME}/.your-taste`;
  const userClaudeMdPath = `${process.env.HOME}/.claude/CLAUDE.md`;
  const thinkingContextPath = `${tasteDir}/thinking-context.md`;
  const baseThinkingPath = new URL('../prompts/base-thinking.md', import.meta.url).pathname;
  const outputPath = process.argv[3] || `${tasteDir}/standalone-CLAUDE.md`;

  // Read thinking context (evolved) or base thinking (cold start)
  let thinkingContent;
  try {
    thinkingContent = await fsReadFile(thinkingContextPath, 'utf8');
  } catch {
    try {
      thinkingContent = await fsReadFile(baseThinkingPath, 'utf8');
      console.log('Note: using base-thinking.md (no evolved thinking-context.md yet)');
    } catch {
      console.error('No thinking content found.');
      process.exit(1);
    }
  }
  ...
  let userContent = '';
  try {
    userContent = await fsReadFile(userClaudeMdPath, 'utf8');
  } catch {
    // No user CLAUDE.md — thinking content only
  }

  // Merge: thinking content first, then user CLAUDE.md
  const parts = [];
  parts.push('<!-- your-taste:standalone — generated by `taste export` -->');
  parts.push('<!-- To update: re-run `taste export` after your-taste learns new patterns -->');
  parts.push('');
  parts.push(thinkingContent.trim());
  parts.push('<!-- your-taste:end-standalone -->');
  ...
  console.log(`Exported standalone CLAUDE.md to: ${outputPath}`);
